...
async function fetchFieldDefinitionType(fieldDefinition : FieldDefinition) : Promise<PersonAttributeTypeResponse | undefined>{
  let apiUrl = '';
...
async function fetchPatientIdentifierTypes(): Promise<Array<FetchedPatientIdentifierType>> {
  const [patientIdentifierTypesResponse, primaryIdentifierTypeResponse] = await Promise.all([
    ...
    cacheAndFetch<{ results: Array<{metadataUuid: string}> }>(`${restBaseUrl}/metadatamapping/termmapping?v=full&code=emr.primaryIdentifierType`),
  ]);
...
async function cacheAndFetch<T = unknown>(url?: string) : Promise<FetchResponse<T>> {
  ...
  if(!url){
    throw new Error('URL is required to fetch data from cacheAndFetch');
  }
...
function mapPatientIdentifierType(
  ...
  //description field : patientIdentifierType.description is not returned , verify Gonzalito
   return {
