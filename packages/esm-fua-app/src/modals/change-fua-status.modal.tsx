...
const ChangeFuaStatusModal: React.FC<ChangeFuaStatusModalProps> = ({
  closeModal,
  fuaRequest,
  onStatusChanged,
}) => {
  ...
  const getAvailableEstados = useCallback(() => {
    ...
    const transitions: Record<number, number[]> = {
      [FUA_ESTADOS.PENDIENTE.id]: [FUA_ESTADOS.EN_PROCESO.id, FUA_ESTADOS.RECHAZADO.id],
      [FUA_ESTADOS.EN_PROCESO.id]: [FUA_ESTADOS.COMPLETADO.id, FUA_ESTADOS.RECHAZADO.id],
      [FUA_ESTADOS.COMPLETADO.id]: [FUA_ESTADOS.ENVIADO.id, FUA_ESTADOS.RECHAZADO.id],
      [FUA_ESTADOS.ENVIADO.id]: [], // Estado final
      [FUA_ESTADOS.RECHAZADO.id]: [FUA_ESTADOS.PENDIENTE.id], // Puede volver a Pendiente
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await setFuaEstado(fuaRequest.uuid, selectedEstadoId, abortController);
  ...
  return (
    <>
      <ModalHeader
        ...
        title={t('changeStatus', 'Cambiar Estado del FUA')}
      />
      <ModalBody>
